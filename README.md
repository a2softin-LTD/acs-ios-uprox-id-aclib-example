...
1. Скопіюйте `u_prox_id_lib.xcframework` у свій проєкт і додайте його в таргет:
   *General → Frameworks, Libraries, and Embedded Content*.

    > [!IMPORTANT]
    > Виберіть **Embed & Sign**. Бібліотека — динамічна (`mh_dylib`), тож із
    > *Do Not Embed* проєкт збереться, але впаде на старті з
    > `dyld: Library not loaded: @rpath/u_prox_id_lib.framework/u_prox_id_lib`.

    `.xcframework` містить зрізи для iOS (пристрій і симулятор) та watchOS
    (пристрій і симулятор) — окремо нічого виключати або лінкувати не треба.
    Ресурсів усередині немає, системні фреймворки (CoreBluetooth, CoreLocation,
    Security) підтягуються автолінком.

    Мінімальні версії: **iOS 13.0**, **watchOS 8.0**, **Xcode 16+**.
...
Демка містить код, який виглядає як частина інтеграції, але до бібліотеки не
належить. Копіюючи його до себе, розумійте, що саме берете.

### DisplayManager (`worker/DisplayManager.h` / `.m`)

Це код **демонстраційного застосунку**, а не бібліотеки. Він потрібен лише для
режиму «відкривати двері по включенню екрана»: реєструє спостерігача Darwin-
нотифікації і перепощує її через `NSNotificationCenter`.

> [!WARNING]
> `com.apple.iokit.hid.displayStatus` — **недокументована** Darwin-нотифікація,
> тобто не публічний API. Вона працює, але застосунки, які її використовують,
> можуть бути відхилені на review в App Store. Це ваш ризик, а не вимога
> бібліотеки: жоден метод `u_prox_id_lib` її не потребує.
>
> Якщо режим «по включенню екрана» вам не потрібен — не переносьте
> `DisplayManager` до себе взагалі. Решта сценаріїв (відкриття по кнопці,
> hands free по iBeacon, отримання ключів) працює без нього.

`BackgroundOpenDoorService` та `AppBLEBackgroundWorker` у `worker/BGWorker.swift`
— теж код демки: це приклад того, як зв'язати CoreLocation, iBeacon і
`requestAccessBackground(keyID:completion:)`, а не готовий до продакшену компонент.
...
Дані AccessKey також містять поля:
- `mobileId: String` — унікальний ідентифікатор мобільного ключа (read-only).
- `timeKeyID: String?` — ідентифікатор тимчасового ключа, якщо він є (read-only).
